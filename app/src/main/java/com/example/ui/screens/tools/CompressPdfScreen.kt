...
fun CompressPdfScreen(
    ...
    val filePickerLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            ...
            selectedFileSize = FileUtils.getFileSize(context, uri)
            outputName = selectedFileName.removeSuffix(".pdf") + "_compressed"
            errorMessage = null
        }
    }
                ...
                if (selectedUri == null) {
                    Card(
                        ...
                        Column(
                            ...
                            Text(
                                text = "Choose a PDF file to reduce its file size",
                    ...
                    Card(
                        ...
                        Row(
                            ...
                            Column(modifier = Modifier.weight(1f)) {
                                ...
                                Text(
                                    text = "Original size: ${FileUtils.formatFileSize(selectedFileSize)}",
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant
                                )
                            }
                ...
                item {
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        CompressionOptionCard(
                            preset = CompressionPreset.MAXIMUM_QUALITY,
                            selected = selectedPreset == CompressionPreset.MAXIMUM_QUALITY,
                            onClick = { selectedPreset = CompressionPreset.MAXIMUM_QUALITY }
                        )

                        CompressionOptionCard(
                            preset = CompressionPreset.BALANCED,
                            selected = selectedPreset == CompressionPreset.BALANCED,
                            onClick = { selectedPreset = CompressionPreset.BALANCED }
                        )

                        CompressionOptionCard(
                            preset = CompressionPreset.SMALLER_SIZE,
                            selected = selectedPreset == CompressionPreset.SMALLER_SIZE,
                            onClick = { selectedPreset = CompressionPreset.SMALLER_SIZE }
                        )
                    }
