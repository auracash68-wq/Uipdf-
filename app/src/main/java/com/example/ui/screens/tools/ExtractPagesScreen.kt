...
fun ExtractPagesScreen(
    ...
    val filePickerLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            ...
            outputName = selectedFileName.removeSuffix(".pdf") + "_extracted"
            errorMessage = null
        }
    }
        ...
        LazyColumn(
                ...
                if (selectedUri == null) {
                    Card(
                        ...
                        Column(
                            ...
                            Text(
                                text = "Choose a PDF file to extract pages from",
            ...
            if (selectedUri != null) {
                item {
                    ...
                    OutlinedTextField(
                        value = pageInput,
                        onValueChange = { pageInput = it; errorMessage = null },
                        label = { Text("Page Numbers (e.g. 1, 3, 5-7)") },
                    ...
                    PrimaryButton(
                        text = stringResource(R.string.btn_extract_action),
                        onClick = {
                            try {
                                val pages = ValidationUtils.parsePageRanges(pageInput, 9999)
                                val uri = selectedUri
                                if (uri != null) {
                                    viewModel.extractPages(context.findActivity(), uri, pages, outputName)
                                }
                            } catch (e: Exception) {
                                errorMessage = e.message
                            }
                        },
                        testTag = "extract_action_button"
                    )
                }
            }
